 ...
 * Local Tour Creator
 * 
 * Allows Explorer accounts to create local experience packages
 */
...
export default function LocalTourCreator({ accountType, onClose, navigation }: Props) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'localTours'), {
        createdBy: user.uid,
      ...
      Alert.alert('Success', 'Local tour created successfully!', [
        { text: 'OK', onPress: onClose },
      ]);
    } catch (error: any) {
      Alert.alert('Error', error.message || 'Failed to create local tour');
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TextInput
        style={styles.input}
        placeholder="Authentic Street Food Walk"
        value={title}
      ...
      <TextInput
        ...
        placeholder="Share your authentic local experience..."
        multiline
        numberOfLines={5}
        value={description}
      ...
      <TextInput
        style={[styles.input, styles.textArea]}
        placeholder="Local markets, street food, hidden gems..."
        multiline
        numberOfLines={3}
        value={highlights}
      ...
      >
        {uploading ? (
          ...
          <Text style={styles.createButtonText}>Create Local Tour</Text>
        )}
      </TouchableOpacity>
    </ScrollView>
  );
